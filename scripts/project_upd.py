...
def main():
	...
	classes = pd.factorize(data_qoutations['label'])[1].tolist()
	weights = []
	min_value = (df.urls.value_counts()[:num_classes]).min()
	for clas in classes:
	    weights.append(min_value/df.urls.value_counts().loc[clas])
